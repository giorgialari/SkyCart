...
exports.filterProducts = async (req, res, next) => {
  try {
    ...
    const products = await productsModel.filterProducts(title, category, rating, price);
    res.json(products);
  } catch (err) {
    next(err);
  }
...
exports.insertCart = async (req, res, next) => {
  try {
    ...
    const result = await productsModel.insertCart(user_id, product_id, quantity);
    res.json(result);
  } catch (err) {
    next(err);
  }
...
exports.updateCart = async (req, res, next) => {
  try {
    const { user_id, product_id, quantity } = req.body;
    const result = await productsModel.updateCart(user_id, product_id, quantity);
    res.json(result);
  } catch (err) {
    next(err);
  }
